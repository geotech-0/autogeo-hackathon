...
export function makeReport(
  ...
  const sections = MEMBER_IDS.map((id) => {
    const m = getModule(state, id),
      r = results[id];
    const sourcePages = state.presetId ? getPreset(state).pages[id] : [];
    const sourceLabel = `흙막이계산서 PDF p${sourcePages.join("~")}`;
    const citation = includeOriginalLinks
      ? `<a href="${esc(sourceBase)}${REAL_DESIGN.source.url}#page=${sourcePages[0]}">${sourceLabel}</a>`
      : `<span>${sourceLabel} · 원본 미포함, 현장 원자료본에서 확인</span>`;
    return `<section><h2>${esc(m.title)} <small>${r.status === "stale" ? "해석 재확인 필요" : r.status === "exceeded" ? "기준 초과" : "채택 기준 만족"}</small></h2><p>${esc(m.material)} · ${esc(m.scope)}</p>${state.presetId ? `<p>원문: ${citation} · ${esc(getPreset(state).label)}</p>` : ""}<h3>입력과 채택 조건</h3><table><thead><tr><th>항목</th><th>입력값</th><th>단위 / 출처</th></tr></thead><tbody>${m.fields.map((f) => `<tr><td>${esc(f.label)}</td><td>${esc(state.members[id].values[f.key])}<br><small>${state.members[id].origins?.[f.key] === "imported_analysis" ? "원문 채택" : "사용자 입력"}</small></td><td>${esc(state.members[id].quantities[f.key]?.unit || f.unit)}${f.group === "analysis" ? ` · ${esc(state.members[id].quantities[f.key].stage)} · ${esc(state.members[id].quantities[f.key].location)} · ${esc(state.members[id].quantities[f.key].evidence)}` : ""}</td></tr>`).join("")}${m.fixedRows.map((row) => `<tr><td>${esc(row[0])} <small>읽기 전용</small></td><td colspan="2">${esc(row[1])}</td></tr>`).join("")}</tbody></table>${r.dependencies.length ? `<p class="link">연결 하중: 앵커 Jf ${num(r.dependencies[0].value)} kN/개 · 앵커 입력 개정 ${r.dependencies[0].revision} · 연결 지문 ${esc(r.dependencies[0].fingerprint)}</p>` : ""}<h3>해석값 단위 변환</h3>${
