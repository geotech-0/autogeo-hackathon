...
- 제공 원본 PDF 9개·주상도 이미지 47개·품질 원본 7개는 로컬에 보존하며 Git/공개 파생 빌드에서 제외한다. 실제 수치·좌표·영상·3D는 공개 범위에 포함한다.
...
## 실자료 공개 승인 — 2026-09-22

- 사용자가 “응 전체 항목 공개로 배포 진행해줘.”라고 승인했다. 범위는 준비된 실제 파생자료 전체 항목이며 제공 원문 PDF·주상도 이미지·품질 원본의 공개 제외를 유지한다.
- GitHub/Vercel 갱신을 진행한다. 배포 완료는 공개 SHA와 HTTP/UI 검수 결과를 확인한 뒤 `CHECKPOINT.md`에 기록한다.
